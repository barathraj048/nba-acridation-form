...
export async function GET() {
  try {
    // Fetch all faculty with ALL relations
    ...
    // Title
    doc.setFontSize(20);
    doc.text("Complete Faculty Report", 105, yPos, { align: "center" });
    yPos += 15;
    ...
    faculties.forEach((faculty, index) => {
      ...
      if (faculty.fundingResearch.length > 0) {
        ...
        faculty.fundingResearch.forEach((funding, idx) => {
          ...
          addText(`   Agency: ${funding.agencyName}, Amount: ₹${funding.amount}, Year: ${funding.yearReceived}`);
          yPos += 3;
        });
        yPos += 3;
      }
      ...
      if (faculty.consulting.length > 0) {
        ...
        faculty.consulting.forEach((cons, idx) => {
          ...
          addText(`   Company: ${cons.companyName}, Amount: ₹${cons.amount}, Year: ${cons.year}`);
      ...
      if (faculty.seminars.length > 0) {
        ...
        faculty.seminars.forEach((sem, idx) => {
          ...
          if (sem.fundingAgency) addText(`   Funding: ${sem.fundingAgency}, Amount: ₹${sem.amount}`);
          yPos += 3;
        });
        yPos += 3;
      }
    ...
    return new NextResponse(pdfBuffer, {
      headers: {
        ...
        "Content-Disposition": "attachment; filename=complete_faculty_report.pdf",
      },
    });
  } catch (error: any) {
